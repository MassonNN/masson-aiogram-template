...
TEST_CHAT = Chat(id=12, type='private', title=None, username=TEST_USER.username,
...
def get_message(text: str, chat=TEST_CHAT, from_user=TEST_USER):
    return Message(message_id=123, date=datetime.now(), chat=chat, from_user=from_user,
                   sender_chat=TEST_CHAT, forward_from=None, forward_from_chat=None, forward_from_message_id=None,
...
def get_chat(
    ...
    return Chat(
        id=id,
        type=type, title=title, username=username,
...
def get_callback_query(data: str | CallbackData, from_user=TEST_USER, message=None):
    return CallbackQuery(id='test', from_user=from_user, chat_instance='test',
