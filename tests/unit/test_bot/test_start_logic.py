...
from bot.logic.start import start
...
async def test_start_response():
    ...
    await start(mocked_message, mocked_translator)

    mocked_message.answer.assert_awaited_once_with(TEST_TEXT)
    mocked_translator.get.assert_called_once_with('welcome')
